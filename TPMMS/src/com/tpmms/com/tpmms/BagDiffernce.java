...
public class BagDiffernce {
	
	 static int itertion=0;
	public static String mergeSort(ArrayList<String> sublist,int differ) {
...
int write=0;
		for (int k = 0; k < sublist.size(); k = k + 2) {
			String list12 =System.getProperty("user.dir") +System.getProperty("file.separator")+"buffer1" +System.getProperty("file.separator")+ itertion + "-sublist-" + k + "_" + (k + 1);
		...
		System.out.println("iteration "+itertion+" in phase 2 merging time taken for this iteration is"+ (System.currentTimeMillis() - itertionStart)+"ms");
		...
		if (mergedFiles.size() > 1) {
			itertion++;
			return mergeSort(mergedFiles,differ+1);
		} else {
